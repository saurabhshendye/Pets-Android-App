...
public class EditorActivity extends AppCompatActivity
    ...
    private final int EDIT_LOADER = 1;

    private boolean ActivityFunctionFlag;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (contentURI!=null){
            // False for editing the pet
            ActivityFunctionFlag = false;
            setTitle("Edit Pet");
            // Get the loader instance
            getLoaderManager().initLoader(EDIT_LOADER,null,this);

        } else {
            // True for adding the pet
            ActivityFunctionFlag = true;
            setTitle("Add Pet");
    ...
    private void savePet(){
        ...
        if (contentURI==null && TextUtils.isEmpty(nameEditable) && TextUtils.isEmpty(breedEditable) &&
                                TextUtils.isEmpty(weightEditable) &&
                mGender == PetsContract.PetEntry.GENDER_UNKNOWN){
            return;
        }

        int weight = 0;
              if (!TextUtils.isEmpty(weightEditable)) {
                        weight = Integer.parseInt(weightEditable);
                    }
        values.put(PetsContract.PetEntry.COLUMN_WEIGHT, weight);
//        // Get writable database
//        PetsDBHelper petsDBHelper = new PetsDBHelper(this);

        // If it is new pet activity then insert the pet else edit(update) the pet
        if (ActivityFunctionFlag){
            ...
            if (rowsUpdated !=0){
                Toast.makeText(getApplicationContext(), "Database successfully updated",
                        Toast.LENGTH_SHORT).show();
            }else {
                Toast.makeText(getApplicationContext(), "Error updating the database",
                        Toast.LENGTH_SHORT).show();
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        if (cursor.moveToFirst()){
            int nameColumn = cursor.getColumnIndex(PetsContract.PetEntry.COLUMN_NAME);
            int breedColumn = cursor.getColumnIndex(PetsContract.PetEntry.COLUMN_BREED);
            int genderColumn = cursor.getColumnIndex(PetsContract.PetEntry.COLUMN_GENDER);
            int weightColumn = cursor.getColumnIndex(PetsContract.PetEntry.COLUMN_WEIGHT);
    ...
    private void deletePet() {
        ...
        if (rowsDeleted !=0) {
            Toast.makeText(getApplicationContext(), "Pet Successfully deleted from the database",
                    Toast.LENGTH_SHORT).show();
